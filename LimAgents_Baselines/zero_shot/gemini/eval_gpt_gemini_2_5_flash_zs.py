import pandas as pd
df = pd.read_csv("/lstr/sahara/datalab-ml/ibrahim/limagents_update/zero_shot/gemini/df_zs_gemini_100_199.csv") 
...
# import re
# # parsing from 7 agents gemini 
# import re

# def parse_merged_limitations(text_str):
#     """
#     Parses consolidated limitation text where:
#     1. Categories are on their own lines wrapped in bold (e.g. **Category**)
#     2. Limitations start with a bullet (* or -) followed by bold title (e.g. * **Title:**)
...
#     # Split into lines to handle the structure row by row
...
#     # Regex patterns
#     # 1. Limitation: Starts with bullet (* or -), whitespace, then **Title**
#     # Group 1: Title, Group 2: Description
#     limitation_pattern = re.compile(r'^[\*\-]\s+\*\*(.*?)\*\*[:\s]*(.*)')
    
#     # 2. Category: Line starts and ends with **, contains no bullets
...
#         # --- Check for Limitation Item first ---
#         lim_match = limitation_pattern.match(line)
#         if lim_match:
#             title = lim_match.group(1).strip()
#             # Remove trailing colon from title if caught inside bolding (e.g. **Title:**)
#             if title.endswith(':'):
#                 title = title[:-1]
                
#             body = lim_match.group(2).strip()
            
#             # Clean body (remove leading colon if regex put it in the body)
#             if body.startswith(':'):
#                 body = body[1:].strip()
...
#             })
#             lim_id += 1
#             continue # Move to next line if we found a limitation

#         # --- Check for Category Header ---
#         # Only check if it wasn't a limitation
#         cat_match = category_pattern.match(line)
#         if cat_match:
#             new_cat = cat_match.group(1).strip()
#             # filter out false positives if necessary (e.g. very long lines)
#             if len(new_cat) < 100: 
#                 current_category = new_cat

#     return limitations

# parsing from 7 agents gpt generated limitations 
# def parse_merged_limitations(text_str):
#     """
