...
df_lim = pd.read_csv("df.csv") 
df_nov = pd.read_csv("df_nov.csv") 

import pandas as pd


# 2. Map those values onto df_lim based on its 'Unnamed: 0' column
df_lim['novelty_lim'] = df_lim['Unnamed: 0'].map(mapping_subset)
 
# using gpt to merge the final limitations and novelty limtations into a single column  
...
def merge_limitations_row(row):
    # Skip if both are empty
    if pd.isna(row['final_merged_limitations']) and pd.isna(row['novelty_lim']):
        return "No limitations found."
    
    prompt = get_merge_prompt(row['final_merged_limitations'], row['novelty_lim'])
...
if 'merged_final_output' not in df_lim.columns:
    df_lim['merged_final_output'] = None

# Iterate and save every 10 rows
for i in range(len(df_lim)):
    # Only process if not already done (useful for resuming)
    if pd.isna(df_lim.loc[i, 'merged_final_output']):
        print(f"Processing row {i}...")
        df_lim.loc[i, 'merged_final_output'] = merge_limitations_row(df_lim.iloc[i])
...
def parse_merged_limitations(text_str):
    ...
    # Pattern for Category: Matches an optional bullet, then text inside ** **
    # E.g., Matches "- **Novelty and Significance**" or "**Novelty**:"
    cat_pat = re.compile(r'^[-*]?\s*\*\*(.*?)\*\*[:\s]*$')

    # Pattern for Limitation: Matches lines starting with a - or *
    lim_pat = re.compile(r'^[-*]\s+(.+)$')

    for ln in lines:
        # Skip the intro text
        if "Consolidated Limitations" in ln:
            continue

        # 1. Check if the line is a Category Header FIRST
        mcat = cat_pat.match(ln)
        if mcat:
            # Extract the text between the ** **, update category, and skip to next line
            current_category = mcat.group(1).strip()
            continue

        # 2. If it's not a category header, check if it's a Limitation Bullet
