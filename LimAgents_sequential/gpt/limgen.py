...
OUTPUT_DIR = "/lstr/sahara/datalab-ml/ibrahim/limagents_update/llm_seq_7_agents/gpt"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "df_gpt4o_mini_7_agents_rows_100_199.csv")
...
# Keep a safety buffer for system prompt + formatting + model overhead + output tokens
SAFE_INPUT_LIMIT = 110000

# Output tokens per agent
MAX_NEW_TOKENS = 900  # adjust if you want longer agent outputs

# Token budgets for paper/citations inside prompts (conservative)
PAPER_TOKEN_BUDGET = 90000
CITATION_TOKEN_BUDGET = 15000
...
def _get_encoding():
    # gpt-4o-mini uses o200k_base (fallback if old tiktoken)
    try:
...
# ============================================================
# 4) GRACEFUL EXIT (emergency save)
# ============================================================
...
# ============================================================
# 5) GPT-4o-mini CALL (replaces run_llama)
# ============================================================

def chat_with_retry(messages, max_retries=8):
    """
    Retries on 429 and transient errors with exponential backoff + jitter.
    """
    for attempt in range(max_retries):
        try:
            ...
            print(f"⚠️ Transient error ({type(e).__name__}). Sleeping {sleep:.1f}s...")
            time.sleep(sleep)
        except APIError as e:
            # Some APIErrors are transient
            sleep = min(60, (2 ** attempt) + random.random())
            print(f"⚠️ APIError. Sleeping {sleep:.1f}s... ({e})")
            time.sleep(sleep)
    raise RuntimeError("Failed after retries.")

def run_gpt(user_prompt: str, system_prompt: str = None) -> str:
    msgs = []
    if system_prompt:
        msgs.append({"role": "system", "content": system_prompt})
    msgs.append({"role": "user", "content": user_prompt})
    return chat_with_retry(msgs)

# ============================================================
# 6) MAIN PIPELINE (sequential 7 agents + master)
# ============================================================
...
def run_pipeline():
    ...
    print(f"Starting sequential 7-agent generation (GPT-4o-mini) for rows {START_ROW}:{END_ROW} ({len(df_slice)} rows)...")
    ...
    for r in tqdm(range(len(df_slice))):
        ...
        citation_text = extract_intro_and_abstract(cited_raw)  # <-- uses your existing cited_in column
        ...
        # Truncate inputs for 128k context safety
        ...
        try:
            ...
            out = run_gpt(p, system_prompt="You are an expert reviewer.")
            ...
            out = run_gpt(p, system_prompt="You are an expert citation analyst.")
            ...
            out = run_gpt(p, system_prompt="You are an expert methodologist.")
            ...
            out = run_gpt(p, system_prompt="You are an expert experimentalist.")
            ...
            out = run_gpt(p, system_prompt="You are an expert on robustness and efficiency.")
            ...
            out = run_gpt(p, system_prompt="You are an expert on clarity and reproducibility.")
            ...
            out = run_gpt(p, system_prompt="You are an expert on data and ethics.")
            ...
            final_out = run_gpt(master_prompt, system_prompt="You are the Master Agent.")
